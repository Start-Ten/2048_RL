...
class PrioritizedReplayBuffer:
    def __init__(self, capacity=100000, alpha=0.6):  # 容量增加到100k
...
class DQNAgent:
    def __init__(self, input_channels, action_size, lr=5e-5, gamma=0.995, 
                 ...
                 target_update_freq=5000, batch_size=512):  # 优化参数
        self.input_channels = input_channels
        ...
        self.optimizer = optim.AdamW(self.policy_net.parameters(), lr=lr, weight_decay=1e-5)  # 使用AdamW
        self.memory = PrioritizedReplayBuffer(100000)  # 更大的缓冲区
        self.steps_done = 0
        self.loss_fn = nn.SmoothL1Loss(reduction='none')
        
        # 混合精度训练工具
        self.scaler = amp.GradScaler(enabled=device.type != 'cpu')  # 自动处理混合精度
        
        # 学习率调度器
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, mode='max', factor=0.5, patience=500
        )
...
# 训练函数（带混合精度训练）
def train_agent(agent, env, episodes=50000, save_path='models/dqn_2048.pth', 
    ...
    for episode in progress_bar:
        ...
        # 更新学习率
        agent.update_learning_rate(avg_score)
...
if __name__ == "__main__":
    args = {"train": 1, "resume": 1, "play": 0, "episodes": 50000}  # 增加训练轮数
    env = Game2048(size=4)
